...
resource "google_iam_workload_identity_pool" "id_pool" {
  ...
  workload_identity_pool_id = "${var.repo}-id-pool"
}
...
resource "google_service_account_iam_member" "pool_impersonation" {
  ...
  member             = "principalSet://iam.googleapis.com/${google_iam_workload_identity_pool.id_pool.name}/attribute.repository/${var.owner}/${var.repo}"
}